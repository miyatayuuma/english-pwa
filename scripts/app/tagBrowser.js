...
const state={items:[],characters:[],catalog:null,activeType:'character',selected:null,query:'',loaded:false,loading:null};
...
function injectStyles(){
  ...
  const style=document.createElement('style');style.id='tagBrowserStyles';style.textContent=`
    ...
    .tag-browser__grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.tag-browser__card{border:1px solid rgba(148,163,184,.14);background:rgba(148,163,184,.04);color:inherit;border-radius:15px;padding:11px;text-align:left;font:inherit;cursor:pointer;min-width:0;min-height:82px}.tag-browser__card.is-selected{border-color:rgba(129,140,248,.62);background:rgba(99,102,241,.13)}.tag-browser__card--person{display:grid;grid-template-columns:46px minmax(0,1fr);gap:9px;align-items:center}.tag-browser__card img{width:46px;height:46px;border-radius:13px;object-fit:cover;background:rgba(148,163,184,.1)}.tag-browser__card-copy{min-width:0}.tag-browser__card strong{display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-size:14px;line-height:1.3}
    .tag-browser__rankline{display:flex;align-items:center;gap:6px;margin-top:4px;font-size:10px}.tag-browser__rank{font-weight:850;color:#c7d2fe}.tag-browser__intimacy{margin-left:auto;opacity:.65}.tag-browser__next{margin-top:4px;font-size:9px;opacity:.52;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.tag-browser__bond{height:5px;margin-top:6px;border-radius:999px;overflow:hidden;background:rgba(148,163,184,.12)}.tag-browser__bond-fill{height:100%;border-radius:999px;background:linear-gradient(90deg,#818cf8,#34d399)}
    ...
    @media(min-width:650px){.tag-browser__grid{grid-template-columns:repeat(3,minmax(0,1fr))}}@media(max-width:390px){.tag-browser__body{padding-inline:12px}.tag-browser__grid{gap:6px}.tag-browser__card{padding:9px}.tag-browser__card--person{grid-template-columns:42px minmax(0,1fr)}.tag-browser__card img{width:42px;height:42px}.tag-browser__tab{font-size:12px}}
...
function relationshipCopy(entry){
  const wrap=document.createDocumentFragment();const line=document.createElement('div');line.className='tag-browser__rankline';
  const rank=document.createElement('span');rank.className='tag-browser__rank';rank.textContent=entry.rank?.label||'知り合い';const intimacy=document.createElement('span');intimacy.className='tag-browser__intimacy';intimacy.textContent=`親密度 ${entry.intimacy??0}`;line.append(rank,intimacy);
  const next=document.createElement('div');next.className='tag-browser__next';next.textContent=entry.rank?.id==='best_friend'?(entry.next?.remaining?`完全定着まであと ${entry.next.remaining}文`:'完全定着'):(entry.next?.remaining?`${entry.next.label}まであと ${entry.next.remaining}文`:`${entry.next?.label||''}までもう少し`);
  const bond=document.createElement('div');bond.className='tag-browser__bond';const fill=document.createElement('div');fill.className='tag-browser__bond-fill';fill.style.width=`${Math.max(0,Math.min(100,Number(entry.intimacy)||0))}%`;bond.appendChild(fill);wrap.append(line,next,bond);return wrap;
}
...
function cardFor(entry){
  ...
  button.addEventListener('click',()=>{state.selected={type:entry.type,id:entry.id};localStorage.setItem(SELECTED_KEY,JSON.stringify(state.selected));renderResults();});return button;
}
...
function detailFor(entry){
  ...
  if(entry.type==='character') copy.appendChild(relationshipCopy(entry));else copy.appendChild(skillProgressMeta(entry));head.appendChild(copy);box.appendChild(head);
  if(entry.type==='character'){
    const stats=document.createElement('div');stats.className='tag-browser__stats';stats.textContent=`担当 ${entry.total}文 · 着手 ${entry.started} · 習得 ${entry.mastered} · 復習 ${entry.due}`;box.appendChild(stats);
    const themes=(entry.themes||[]).map(theme=>theme.label).join('・');const summary=entry.profile?.summary_ja||entry.profile?.archetype_ja||'';if(themes||summary){const p=document.createElement('p');p.className='tag-browser__summary';p.textContent=[themes?`得意テーマ：${themes}`:'',summary].filter(Boolean).join('。');box.appendChild(p);}
  }else{
  ...
  const start=document.createElement('button');start.type='button';start.className='tag-browser__start';start.textContent=entry.type==='character'?`${entry.label}と遊ぶ`:'このスキルを練習する';start.disabled=entry.total<=0;start.addEventListener('click',()=>startScopedStudy(entry));box.appendChild(start);return box;
}
...
function renderResults(){
  ...
  const selected=findSelected();if(selected&&selected.type===state.activeType&&entries.some(entry=>entry.id===selected.id)){const detail=detailFor(selected);if(detail) host.appendChild(detail);}
}
...
function renderShell(){
  const dialog=ensureDialog();const body=dialog.querySelector('.tag-browser__body');body.replaceChildren();const tabs=document.createElement('div');tabs.className='tag-browser__tabs';
  for(const type of BROWSE_TYPES){const button=document.createElement('button');button.type='button';button.dataset.type=type;button.className='tag-browser__tab'+(state.activeType===type?' is-active':'');button.textContent=type==='character'?'キャラ':'トレーニング';button.addEventListener('click',()=>setActiveType(type));tabs.appendChild(button);}body.appendChild(tabs);
...
async function openBrowser(type){
  if(BROWSE_TYPES.includes(type)) state.activeType=type;injectStyles();const dialog=ensureDialog();if(!dialog.open) dialog.showModal();const body=dialog.querySelector('.tag-browser__body');if(!state.loaded) body.innerHTML='<div class="tag-browser__loading">読み込んでいます…</div>';
...
function isBrowserEntry(target){
  if(!(target instanceof Element)) return false;if(target.closest('[data-course="tag"]')) return true;if(target.closest('.explore-tag-link')) return true;const button=target.closest('#focusHomeNav button');return !!button&&['キャラ・スキル','キャラを選ぶ','トレーニング'].includes(button.textContent?.trim());
}
function interceptEntrances(event){
  if(!isBrowserEntry(event.target)) return;event.preventDefault();event.stopImmediatePropagation();document.getElementById('learningChoiceDialog')?.close();document.getElementById('focusExploreDialog')?.close();document.getElementById('focusTagDialog')?.close();const label=event.target.closest('button')?.textContent?.trim();const type=label==='トレーニング'?'skill':(label==='キャラを選ぶ'?'character':undefined);setTimeout(()=>openBrowser(type),0);
}
