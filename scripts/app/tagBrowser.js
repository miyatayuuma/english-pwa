...
function cardFor(entry){
  ...
  if(entry.type==='character'){
    ...
    const small=document.createElement('small');small.className='tag-browser__themes';
    small.textContent=themes?`${themes} · ${entry.total}文 · 習得 ${mastery}%`:`${entry.total}文 · 習得 ${mastery}%`;
    text.append(name,small);button.append(img,text);
  }else{
    const name=document.createElement('strong');name.textContent=entry.label;
    const small=document.createElement('small');small.textContent=`${entry.total}文 · 習得 ${mastery}%`;
...
function detailFor(entry){
  ...
  sub.textContent=`${entry.total}文 · 未学習 ${entry.fresh} · 学習中 ${entry.learning} · 復習 ${entry.due}`;
