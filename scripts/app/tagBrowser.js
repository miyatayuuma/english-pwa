...
const state={items:[],characters:[],catalog:null,activeType:'character',selected:null,query:'',loaded:false,loading:null,characterOnly:false,characterDetailId:''};
...
function cardFor(entry){
  ...
  button.addEventListener('click',()=>{state.selected={type:entry.type,id:entry.id};if(entry.type==='character') state.characterDetailId=entry.id;localStorage.setItem(SELECTED_KEY,JSON.stringify(state.selected));renderResults();});return button;
}


function characterDetailFor(entry){
  const box=document.createElement('section');box.className='tag-browser__detail tag-browser__detail--character';
  const back=document.createElement('button');back.type='button';back.className='tag-browser__detail-back';back.textContent='← 一覧に戻る';back.addEventListener('click',()=>{state.characterDetailId='';renderResults();});box.appendChild(back);
...
function renderResults(){
  ...
  if(characterDetail){host.appendChild(characterDetailFor(characterDetail));return;}
