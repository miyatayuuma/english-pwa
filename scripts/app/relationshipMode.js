...
function injectStyles(){
  ...
  style.textContent=`
    ...
    .friendship-hero__copy{min-width:0}.friendship-hero__name{font-size:clamp(28px,9vw,38px);font-weight:950;letter-spacing:-.035em;line-height:1}.friendship-rank{--rank-accent:#cbd5e1;display:inline-flex;margin-top:8px;padding:5px 10px;border:1px solid color-mix(in srgb,var(--rank-accent) 34%,transparent);border-radius:999px;background:color-mix(in srgb,var(--rank-accent) 12%,transparent);color:var(--rank-accent);font-size:11px;font-weight:900}
...
function renderHome(){
  ...
  const rank=document.createElement('div');rank.className='friendship-rank';rank.style.setProperty('--rank-accent',relationshipRankColor(rel.rank.id));rank.textContent=rel.rank.label;
  ...
  if(nav){
    ...
    const chars=document.createElement('button');chars.type='button';chars.className='friendship-nav__choose';chars.textContent='連絡先';chars.addEventListener('click',()=>globalThis.__OPEN_ENGLISH_LEARNING_BROWSER__?.('character'));
...
function renderReviewSummary(){
  ...
  if(result.relationship){const rank=document.createElement('div');rank.className='friendship-review__rank';rank.style.setProperty('--rank-accent',relationshipRankColor(result.rankAfter.id));rank.dataset.rankUp=String(result.rankUp);rank.textContent=result.rankUp?`${result.rankBefore.label} → ${result.rankAfter.label}`:result.rankAfter.label;heroCopy.appendChild(rank);}hero.append(portrait,heroCopy);panel.appendChild(hero);
