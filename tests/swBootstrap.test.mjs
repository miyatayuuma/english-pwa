...
test('a versioned bootstrap escapes stale cache-first workers and registers updates',()=>{
  assert.match(index,/swBootstrap\.js\?v=5\.43/);
  assert.match(bootstrap,/swUpdatePrompt\.js\?v=5\.43/);
...
test('the current worker precaches its update bootstrap',()=>{
  assert.match(worker,/version\.js\?v=5\.43/);
  assert.match(worker,/swBootstrap\.js\?v=5\.43/);
  assert.match(worker,/swUpdatePrompt\.js\?v=5\.43/);
